...
export function buildLegacyOfferPayload(offerForm, productsByCategory = {}, inventory = []) {
  ...
  const productsIncluded = selectedProducts.map((product) => ({
    ...
    title: product.title,
    price: product.price,
    cost: Number(product.purchasePrice ?? product.purchase_price ?? product.cost ?? 0) || 0,
    unitCost: Number(product.purchasePrice ?? product.purchase_price ?? product.cost ?? 0) || 0,
    purchasePrice: Number(product.purchasePrice ?? product.purchase_price ?? product.cost ?? 0) || 0,
