...
import { CalendarRange, CheckCircle2, ClipboardList, CreditCard, FileText, Plus, ReceiptText, Search, Trash2, Wallet, X } from 'lucide-react';
...
const PRIMARY_BUTTON_CLASS = 'inline-flex items-center gap-1 rounded-xl border border-sky-200 bg-sky-50 px-2.5 py-1.5 text-[10px] font-black uppercase tracking-[0.1em] text-sky-700 transition hover:bg-sky-100 disabled:cursor-not-allowed disabled:opacity-40';
const NEUTRAL_BUTTON_CLASS = 'inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-white px-2.5 py-1.5 text-[10px] font-black uppercase tracking-[0.1em] text-slate-600 transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-800 disabled:cursor-not-allowed disabled:opacity-40';
const DANGER_BUTTON_CLASS = 'inline-flex items-center gap-1 rounded-xl border border-rose-200 bg-rose-50 px-2.5 py-1.5 text-[10px] font-black uppercase tracking-[0.1em] text-rose-700 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-40';
const SUCCESS_BUTTON_CLASS = 'inline-flex items-center gap-1 rounded-xl border border-emerald-200 bg-emerald-50 px-2.5 py-1.5 text-[10px] font-black uppercase tracking-[0.1em] text-emerald-700 transition hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-40';

const normalizeLegacyOrderStatus = (value = '') => String(value || '').replace('Señado', 'Señado').replace('SeÃ±ado', 'Señado');
...
function RecordCard({ record, isActive, onSelect }) {
  ...
  return (
    <button type="button" onClick={onSelect} className={`w-full rounded-[16px] border px-2.5 py-2 text-left transition ${isActive ? 'border-sky-200 bg-white shadow-[0_6px_18px_rgba(148,163,184,0.12)] ring-1 ring-sky-100' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50'}`}>
      <div className="flex items-start justify-between gap-2.5">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full border border-slate-200 bg-white px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-slate-600">{RECORD_TYPE_LABELS[record.type]}</span>
            <span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${STATUS_STYLES[statusLabel] || STATUS_STYLES.Pendiente}`}>{statusLabel}</span>
            <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1 font-mono text-[10px] font-black text-slate-500">{formatRecordCode(record)}</span>
          </div>
          <p className="mt-1 truncate text-[12px] font-black text-slate-800">{record.customerName}</p>
          <p className="text-[11px] font-semibold text-slate-500">{record.customerKind} · {record.customerPhone}</p>
          {record.eventLabel && <p className="mt-1.5 truncate text-[11px] font-semibold text-slate-500">{record.eventLabel}</p>}
        </div>
        <div className="text-right">
          <p className="text-[11px] font-semibold text-slate-400">{formatDateAR(record.createdAt)}</p>
          <p className="mt-1.5 text-[13px] font-black text-slate-800"><FancyPrice amount={record.totalAmount} /></p>
        </div>
      </div>
      <div className="mt-2 grid gap-x-2 gap-y-1 text-[10px] font-semibold text-slate-500 sm:grid-cols-3">
...
export default function OrdersView({ budgets, orders, members, inventory, categories, onCreateBudget, onUpdateBudget, onDeleteBudget, onDeleteOrder, onConvertBudgetToOrder, onRegisterOrderPayment, onCancelOrder, onMarkOrderRetired, onPrintRecord }) {
  ...
  const handleSaveBudget = async (payload) => {
    ...
    try {
      if (editingBudget) await onUpdateBudget(editingBudget.id, payload);
  ...
  return (
    <>
      <div className="grid h-full min-h-0 gap-0 xl:grid-cols-[330px_minmax(0,1fr)]">
            ...
            <div className="border-b border-slate-200 px-2.5 py-2">
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  ...
                  className="inline-flex shrink-0 items-center gap-1.5 rounded-2xl border border-sky-200 bg-sky-50 px-2.5 py-1.5 text-[10px] font-black uppercase tracking-[0.1em] text-sky-700 transition hover:bg-sky-100"
                >
                  <Plus size={11} />
                  Crear presupuesto
                </button>
        ...
        <div className="min-h-0 bg-slate-50">
          {selectedRecord ? (
            <div className="flex h-full min-h-0 flex-col">
              ...
              <div className="grid min-h-0 flex-1 gap-1.5 p-2 lg:grid-cols-[0.72fr_1.28fr]">
                <div className="min-h-0 overflow-y-auto rounded-[18px] border border-slate-200 bg-white p-2.5 scrollbar-hide">
                  <p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Resumen operativo</p>
                  <div className="mt-2 grid gap-1.5 sm:grid-cols-2">
                    <div className="rounded-[14px] border border-slate-200 bg-slate-50 p-2"><p className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">ID</p><p className="mt-1 font-mono text-[12px] font-black text-slate-800">{formatRecordCode(selectedRecord)}</p></div>
                    <div className="rounded-[14px] border border-slate-200 bg-slate-50 p-2"><p className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">{selectedRecord.type === 'order' ? 'Documento origen' : 'Documento'}</p><p className="mt-1 text-[12px] font-black text-slate-800">{selectedRecord.documentTitle || 'PRESUPUESTO'}</p></div>
                    <div className="rounded-[14px] border border-slate-200 bg-slate-50 p-2"><p className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">Creado</p><p className="mt-1 text-[12px] font-black text-slate-800">{formatDateAR(selectedRecord.createdAt)}</p></div>
                    <div className="rounded-[14px] border border-slate-200 bg-slate-50 p-2"><p className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">Retiro</p><p className="mt-1 text-[12px] font-black text-slate-800">{formatPickupDate(selectedRecord.pickupDate)}</p></div>
                    <div className="rounded-[22px] border border-emerald-200 bg-emerald-50 p-3"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-emerald-500">Seña</p><p className="mt-1 text-lg font-black text-emerald-700"><FancyPrice amount={selectedRecord.depositAmount || 0} /></p></div>
                    <div className="rounded-[18px] border border-sky-200 bg-sky-50 p-2.5"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-sky-500">Abonado</p><p className="mt-1 text-[16px] font-black text-sky-700"><FancyPrice amount={selectedRecord.paidTotal || 0} /></p></div>
                    <div className="rounded-[18px] border border-amber-200 bg-amber-50 p-2.5"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-amber-500">Restante</p><p className="mt-1 text-[16px] font-black text-amber-700"><FancyPrice amount={selectedRecord.remainingAmount || 0} /></p></div>
                    <div className="rounded-[18px] border border-slate-200 bg-slate-50 p-2.5"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-slate-400">Total</p><p className="mt-1 text-[16px] font-black text-slate-800"><FancyPrice amount={selectedRecord.totalAmount || 0} /></p></div>
                  </div>
                  <div className="mt-2 rounded-[14px] border border-slate-200 bg-slate-50 p-2.5">
                    <p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">Cliente</p>
                    <p className="mt-1 text-[13px] font-black text-slate-800">{selectedRecord.customerName}</p>
                    <p className="mt-1 text-[11px] font-semibold text-slate-500">{selectedRecord.customerPhone}</p>
                    <p className="mt-1 text-[11px] font-semibold text-slate-500">{selectedRecord.customerKind}</p>
                    {selectedRecord.customerNote && <><p className="mt-2.5 text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Nota</p><p className="mt-1 text-[12px] font-medium text-slate-600">{selectedRecord.customerNote}</p></>}
                    {linkedOrderForBudget && <><p className="mt-2.5 text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Pedido vinculado</p><p className="mt-1 font-mono text-[12px] font-semibold text-slate-600">{formatRecordCode(linkedOrderForBudget)}</p></>}
                  </div>
                </div>
                <div className="min-h-0 overflow-y-auto rounded-[18px] border border-slate-200 bg-white p-2.5 scrollbar-hide">
                  ...
                  <div className="mt-2 space-y-1">
                    {selectedRecord.items.map((item) => {
                      ...
                      return <div key={item.id} className="rounded-[14px] border border-slate-200 bg-slate-50 px-2.5 py-2">
                        <div className="flex items-start justify-between gap-2.5">
                          <div className="min-w-0"><p className="text-[12px] font-black text-slate-800">{item.title}</p><p className="mt-0.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-slate-400">{item.category}</p></div>
                          <div className="text-right"><p className="text-[12px] font-black text-slate-800">{formatCurrency(subtotal)}</p><p className="text-[10px] font-semibold text-slate-500">{formatBudgetItemQuantity(item)}</p></div>
                        </div>
                        <div className="mt-1.5 flex flex-wrap items-center gap-1 text-[10px] font-semibold text-slate-500">
                          <span className="rounded-full bg-white px-2 py-1">Unitario: {formatCurrency(item.newPrice)}{item.product_type === 'weight' ? ' /Kg' : ''}</span>
                          {item.productId ? <span className="rounded-full bg-white px-2 py-1">ID #{item.productId}</span> : <span className="rounded-full bg-white px-2 py-1">Ítem manual</span>}
                        </div>
                      </div>;
                    })}
                  </div>
      ...
      <BudgetBuilderModal isOpen={isBudgetModalOpen} onClose={() => { setIsBudgetModalOpen(false); setEditingBudget(null); }} inventory={inventory} categories={categories} members={members} initialRecord={editingBudget} onSave={handleSaveBudget} isSaving={isSavingBudget} />
