-- Bridge between Rebu app users and Supabase Auth.
-- Idempotent and additive: does not change the current login flow by itself.
-- After linking app_users.auth_email/auth_user_id to auth.users, the frontend can open
-- a real Supabase Auth session and call RPCs granted only to authenticated.

create extension if not exists citext;

alter table if exists public.app_users
  add column if not exists auth_user_id uuid null,
  add column if not exists auth_email citext null;

create unique index if not exists app_users_auth_user_id_key
  on public.app_users (auth_user_id)
  where auth_user_id is not null;

create unique index if not exists app_users_auth_email_key
...
revoke all on function public.verify_app_user_login_auth_bridge(uuid, text) from public;
revoke all on function public.get_current_auth_app_user() from public;
revoke all on function public.link_current_auth_user_to_app_user(uuid, text) from public;

grant execute on function public.verify_app_user_login_auth_bridge(uuid, text) to anon, authenticated;
grant execute on function public.get_current_auth_app_user() to authenticated;
grant execute on function public.link_current_auth_user_to_app_user(uuid, text) to authenticated;
