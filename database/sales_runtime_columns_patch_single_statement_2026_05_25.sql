...
begin
  if to_regclass('public.sales') is not null then
    ...
    execute 'create index if not exists sales_created_at_idx on public.sales (created_at desc)';
    execute 'create index if not exists sales_client_id_idx on public.sales (client_id)';
    execute 'create index if not exists sales_status_idx on public.sales (status)';
    execute 'create index if not exists sales_user_id_idx on public.sales (user_id)';
    execute 'create index if not exists sales_voided_at_idx on public.sales (voided_at desc) where voided_at is not null';
  end if;
  ...
  if to_regclass('public.sale_items') is not null then
    ...
    execute 'create index if not exists sale_items_sale_id_idx on public.sale_items (sale_id)';
    execute 'create index if not exists sale_items_product_id_idx on public.sale_items (product_id)';
  end if;
